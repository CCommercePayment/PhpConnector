...
class CardApi
{
    ...
    {
        list($response) = $this->v1CardCreditHolderPostWithHttpInfo($auth_token, $request);
        return $response;
    }
     ...
     */
    public function v1CardCreditHolderPostAsync($auth_token, $request = null)
    {
        return $this->v1CardCreditHolderPostAsyncWithHttpInfo($auth_token, $request)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     */
    protected function v1CardCreditHolderPostRequest($auth_token, $request = null)
    {
        // verify the required parameter 'auth_token' is set
        if ($auth_token === null || (is_array($auth_token) && count($auth_token) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $auth_token when calling v1CardCreditHolderPost'
            );
        }

        $resourcePath = '/v1/card/creditHolder';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
